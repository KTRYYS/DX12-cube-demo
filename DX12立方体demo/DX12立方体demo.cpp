...
public:
	...
	ComPtr<IDXGIAdapter> Adapter;                                          //GPU系统（驱动）接口
	...
	ComPtr<ID3D12PipelineState> PSO;                                       //流水线状态
	
	UINT IndexCount;                                                       //索引总数

	UINT CBSize;                                                           //常量缓冲区大小
	void* CBSubresourcePtr;                                                //常量缓冲区子资源地址
	...
	//摄像机的球坐标系
	float Radius = 5;                                                      //球坐标系半径
	float Azimuth = 0;                                                     //水平角
	float Eleva = 0;                                                       //仰角

	//窗口更新
	int downx = 0;                                                         //按下左键后当前鼠标x坐标
	int downy = 0;                                                         //按下左键后当前鼠标y坐标

	//GPU系统
	UINT GPUIndex = 1;                                                     //GPU系统索引，由于本程序运行在带独显笔记本上，为使用独显，设置为1
	UINT GPUNodeMask;                                                      //GPU系统内物理GPU掩码值，单显卡默认为0

	//其它
	static D3DAPP* myApp;                                                  //指向本D3DAPP类的静态指针，在类外初始化
...
public:
	...
	void FlushCommandQueue();                                              //等待命令队列刷新

	//鼠标或窗口大小更新处理
	static void ProcessMouse(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam);                //接收鼠标数据并处理
	void LeftDown(int x,int y);                                            //处理左键按下
	void LeftUp();                                                         //处理左键松开
	void MouseMove(WPARAM LeftState, int x, int y);                        //处理鼠标移动
	void WheelRoll(int x);                                                 //处理滚轮
	void Resize(int width, int height);                                    //改变窗口大小
...
D3DAPP* D3DAPP::myApp = nullptr;                                //初始化静态指针

//窗口过程
LRESULT WinProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	D3DAPP::ProcessMouse(hwnd, msg, wparam, lparam);            //将数据交由
...
D3DAPP::D3DAPP(HWND Hwnd, UINT clientwidth, UINT clientheight)
{
	...
	//LogAdapters();                                     //枚举当前系统中的Adapter

	//创建适配器
	D3D12CreateDevice(Adapter.Get(),                     //指定使用的显示适配器
...
void D3DAPP::CreateCommandObjects()
{
	...
	QueueDesc.NodeMask = GPUNodeMask;                                                //单GPU设置为0
	...
	//创建命令列表
	Device->CreateCommandList(GPUNodeMask,                                           //单GPU系统设置为0
...
void D3DAPP::CaculateWorldViewProjMat()
{
	...
	//计算观察矩阵
	//摄像机参数
	float x = Radius * cosf(Eleva) * cosf(Azimuth);
	float y = Radius * sinf(Eleva);
	float z = Radius * cosf(Eleva) * sinf(Azimuth);

	XMVECTOR CameraPos = XMVectorSet(x, y, z, 1.0f);            //摄像机坐标
...
void D3DAPP::CreateConstantBuffer()
{
	...
	CBSize = (UINT)sizeof(WorldViewProjData) + 255 & ~255;                //常量元素的内存大小，必须为256的整数倍
...
void D3DAPP::UpdateMat()
{
	//将常量符合矩阵复制到常量缓冲区（上传堆）
	memcpy(CBSubresourcePtr, &WorldViewProjData, CBSize);
}
...
void D3DAPP::CreateCBVHeap()
{
	...
	CBVHeapDesc.NodeMask = GPUNodeMask;                                                  //单显卡设置为0
...
void D3DAPP::CreateRootSignature()
{
	...
	//创建根签名
	Device->CreateRootSignature(GPUNodeMask,                                             //单GPU设置为0
...
void D3DAPP::FlushCommandQueue()
{
	...
	//}
	while (Fence->GetCompletedValue() < CurrentFence)
	{
		;
	}
}

void D3DAPP::CmdReset()
{
	//命令列表和命令分配器复用
	CmdAllocator->Reset();
	CmdList->Reset(CmdAllocator.Get(), PSO.Get());
}
...
void D3DAPP::Draw()
{
	...
	CD3DX12_RESOURCE_BARRIER BackBufferBarrier1 =                              //后台缓冲区屏障
		CD3DX12_RESOURCE_BARRIER::Transition(SwapChainBuffer[BackBufferIndex].Get(),
...
void D3DAPP::Run()
{
	CaculateWorldViewProjMat();
	UpdateMat();
	CmdReset();
	Draw();
}

void D3DAPP::Resize(int width, int height)
{
	if (Device)
	{
		ClientWidth = width;
		...
		//重建渲染目标缓冲区
		for (int i = 0; i < SwapChainBufferCount; i++)                             //释放渲染目标缓冲区
		{
			SwapChainBuffer[i].Reset();                                            //释放接口
		}

		SwapChain->ResizeBuffers(SwapChainBufferCount,                             //重置交换链
			ClientWidth, ClientHeight, RTFormat,
		...
		Device->CreateDepthStencilView(DSBuffer.Get(),                           //创建DSV
			nullptr,
...
void D3DAPP::ProcessMouse(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	switch (msg)
	{
	//左键按下
	...
	case WM_SIZE:
	{
		if (myApp)
		{
			myApp->Resize(LOWORD(lparam), HIWORD(lparam));
		}
...
void D3DAPP::LeftDown(int x, int y)
{
	downx = x;
	downy = y;
	SetCapture(hwnd);
...
void D3DAPP::MouseMove(WPARAM LeftState,int x,int y)
{
	if (LeftState&WM_LBUTTONDOWN)
	{
		float differx = 0.01f * (x - downx);
		float differy = 0.01f * (y - downy);
		downx = x;
		downy = y;
		...
		Eleva = Eleva > (XM_PI / 2 - 0.01) ? XM_PI / 2 - 0.01 : Eleva;
		Eleva = Eleva < (-XM_PI / 2 + 0.01) ? -XM_PI / 2 + 0.01 : Eleva;
